...
function startConsumer() {
  ...
  pipe.setAttribute("height", 0.7);
  pipe.setAttribute("width", 6);
  pipe.setAttribute("depth", 0.3);
  ...
  processor.setAttribute("position", { y: -0.7, z: 0.1 });
  ...
  processor.setAttribute("color", "grey");
  pipe.appendChild(processor);
  processor.setAttribute("position", { y: 0.8, z: 0.1 });
...
function sendMessage(protocol) {
  ...
  if (protocol == "mqtt") consumeMqttEvent(num, process);
  else if (protocol == "http") consumeHttpEvent(num, process);
...
function setCameraFocus(xpos, duration) {
  ...
  if (res2 == "object") console.log("res2 is object!");
  ...
  if (typeof xpos == "object") target = xpos;
...
function consumeEvent(item) {
  ...
  msg.setAttribute("height", 0.5);
  msg.setAttribute("width", 0.5);
  msg.setAttribute("depth", 0.2);
  ...
  target = { x: 3 - 0.6, y: posY, z: 0 };

  msg.setAttribute("animation__2", {
    property: "position",
    dur: "3000",
    delay: 950,
    from: from,
  ...
  msg.setAttribute("animation__color", {
    property: "color",
    dur: "3000",
    delay: 1150,
    // from: 'red',
...
function aiResult(item, msgOriginal, process) {
  ...
  //text label child
  msg.firstChild.setAttribute("animation", {
    property: "opacity",
    dur: "2000",
    delay: 0,
    from: 1,
...
function priceResult(item, msgOriginal, process) {
  ...
  // msg.setAttribute('position', {x: -8, y: posY, z: 0})
  msg.setAttribute("position", { x: 0.5, y: -3, z: 0 });
  // msg.setAttribute('src', "#valid")
...
function createZip() {
  ...
  msg.setAttribute("position", { x: 9.5 - 0.3, y: 0, z: 0 });
  msg.setAttribute("height", 0.5);
  msg.setAttribute("width", 0.5);
  msg.setAttribute("depth", 0.2);
  msg.setAttribute("side", "double");
  ...
  number.setAttribute("scale", "2 2 2");
  msg.appendChild(number);
  number.setAttribute("position", { x: -0.05, y: -0.5, z: 0 });
...
function renderEventUnZip(item, process) {
  //, noZipOnFlight)
  posY = 0;
  var msg;
...
function renderPipelineStart(numS3files) {
  ...
  signal.firstElementChild.setAttribute("animation", {
    property: "opacity",
    dur: "2000",
    from: 1,
    to: 0,
  ...
  pipe.setAttribute("opacity", 0.9);

  pipe.setAttribute("animation", {
    property: "rotation",
    dur: "3000",
    from: "0 0 0",
    to: "360 0 0",
  ...
  // let num = 10
  for (let i = 0; i < numS3files; i++) renderPipelineFile(num++);
}
...
function renderPipelineModelReady(item) {
  ...
  msg.setAttribute("radius", 0.3);
  msg.setAttribute("side", "double");
  // msg.setAttribute('color', "#00FF00")
  msg.setAttribute("color", "lightskyblue");
  msg.setAttribute("opacity", 0.7);
  ...
  label.setAttribute("align", "center");
  label.setAttribute("scale", "2 2 2");
  label.setAttribute("position", { y: 0.9 });
  msg.appendChild(label);

  let target = { x: S3_CENTRAL_MODELS, y: 1.5, z: 0 };
  ...
  let from = target;
  target = { x: S3_CENTRAL_MODELS, y: 3, z: 0 };
