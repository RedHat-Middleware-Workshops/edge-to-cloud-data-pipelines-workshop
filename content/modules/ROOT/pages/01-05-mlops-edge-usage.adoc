...
== Closing Thoughts
Congratulations, you did it! You just completed the first Lab.  This MLOps lab took you on a journey inside OpenShift that had you build out an OpenShift AI Project, which included the powerful Jupyter IDE.  You were able to load and run notebooks that focused on leveraging a TensorFlow approach to transfer learning in order to add products to our store's detection model.  Our "edge" shopping application was then able to use that operationalized model with a test image.

Let's continue the journey by clicking on the *Next* link below to take you into lab 2, where you will move into the application development and application services side of this use case.
